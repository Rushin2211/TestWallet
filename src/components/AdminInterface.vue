  ...
  <div id="app">
    ...
    <p v-for="(item, index) in TotalNFTGroup" :key="item">
      <button name="NFTButton"
        ...
        <br/>
        <small class="BtnText">Token ID:{{ item }} &nbsp; AP ID:{{ AP_IDs[index] }}</small>
        <br/>
    ...
    <div v-if="isSelling" class="loading-overlay">
      <div class="loading-icon">
        <img src="../assets/selling-icon.svg" alt="Loading">
      </div>
      <div class="loading-text">For sale, please wait...</div>
    </div>
    ...
    <el-dialog title="Sell NFTs" v-model="directSellFlag" width="380px" style="margin-top: 50px">
      <p style="text-align: center; margin: 10px 0;">Please select the NFTs you want to sell:</p>

      <div style="display: flex; flex-wrap: wrap;">
        <el-checkbox-group v-model="selectedNFTsForSellGroup">
          <el-checkbox :label="item" v-for="item in availableForSaleGroup" :key="item">Token ID:{{ item }}</el-checkbox>
        </el-checkbox-group>
      </div>

      <el-form :model="SellNFTForm" label-width="80px" style="margin-top: 20px;">
        <el-form-item label="Price">
          <el-input v-model="SellNFTForm.price" autocomplete="off"
                    placeholder="Pricing (e.g., 0.1) uint:ether" style="width: 250px"></el-input>
        </el-form-item>
        ...
        <el-form-item label="Duration">
          <el-col :span="10">
            <el-input
                style="width: 100%;"
                v-model="SellNFTForm.durationDay"
          ...
          <el-col :span="10">
            <el-input
                style="width: 100%;"
                v-model="SellNFTForm.durationHour"
      ...
      <div style="text-align: center; margin-top: 20px;">
        <el-button @click="Cancel" style="margin-right: 20px; font-size: 16px;">Cancel</el-button>
        <el-button type="primary" @click="submitInventoryForSale" style="font-size: 16px; padding: 12px 20px;">Confirm
        </el-button>
      </div>
    </el-dialog>
    ...
    <div id="foot">
      ...
      <br/>
      <br/>
      <button name="SellingNFT" class="btn" @click="confirmSellNFTs">Selling NFTs</button>
      <br/>
      <br/>
      <button name="AuctionNFT" class="btn" @click="confirmAuctionNFT">Auction</button>
      <br/>
      <br/>
      <button name="NFTStore" class="btn" @click="NFTStore">NFT Store</button>
      <br/>
      <br/>
      <button name="AuctionInfo" class="btn" @click="goToAuctionInfoPage">Auction Info</button>
      <br/>
      <br/>
      <button name="ConnectionState" class="btn" v-on:click="ConnectionState">Connection State</button>
      <br/>
      <br/>
...
export default {
  ...
  methods: {
    ...
    async submitInventoryForSale() {
      const durationValue = this.TimeConversion(this.SellNFTForm.durationDay, this.SellNFTForm.durationHour);

      let priceValue = Number(this.SellNFTForm.price)
      ...
      if (!(
          this.SellNFTForm.durationDay >= 0 &&
          this.SellNFTForm.durationHour >= 0 &&
      ...
      if (priceValue == null) {
        this.showError("Price cannot be null");
        return
      }
      ...
      if (this.selectedNFTsForSellGroup.length === 0) {
        this.showError("Select the NFT quantity to sell to be greater than 0");
        return
      }
      try {
        priceValue = web3.utils.toWei(priceValue, 'ether');
        this.directSellFlag = false;
        this.isSelling = true;
        const txCount = await web3.eth.getTransactionCount(this.Address);
        const sellInstance = this.contractInstance.methods.listNFTForSale(this.selectedNFTsForSellGroup,
            priceValue, durationValue)
        const sellInstanceEncodedABI = sellInstance.encodeABI();
        const txObject = {
          nonce: txCount,
          to: contractAddress,
          gasLimit: 3000000,
          gasPrice: web3.utils.toWei('2', 'gwei'),
          data: sellInstanceEncodedABI
        };
        const signed = await web3.eth.accounts.signTransaction(txObject, savedAdminPrivateKey);
        await web3.eth.sendSignedTransaction(signed.rawTransaction);
        await this.getOwnedAllNFT(this.Address);
        ...
        Swal.fire({
          position: "center",
          icon: "success",
          title: "The NFTs you selected has been listed on the NFT store!",
          showConfirmButton: false,
          timer: 2500
        });
      } catch (error) {
    ...
    confirmSellNFTs() {
      this.directSellFlag = true;
      this.selectedNFTsForSellGroup = [];
      this.SellNFTForm.price = null;
      this.SellNFTForm.durationDay = null;
      this.SellNFTForm.durationHour = null;
    },
    ...
    ConnectionState() {
      this.$router.push('/ConnectionState')
    },
