...
export default {
    ...
    methods: {
        ...
        // todo
        async getCurrentAuctionNFTs(address) {
            await this.fetchCurrentAuctionNFTs(address);
            setInterval(async () => {
                await this.fetchCurrentAuctionNFTs(address);
            }, 5000);
        },
        ...
        // todo
        async fetchCurrentAuctionNFTs(address) {
            this.currentAuctionNFTsResult = await this.contractInstance.methods.getCurrentAuctionNFTs(address).call();
            if (this.currentAuctionNFTsResult && typeof this.currentAuctionNFTsResult === 'object') {
                this.nftIds = this.currentAuctionNFTsResult[0];
                this.allAuctionNFTNumber = this.nftIds.length;
                for (let i = 0; i < this.allAuctionNFTNumber; i++) {
                    const nftId = this.nftIds[i];
                    const ApId = this.currentAuctionNFTsResult[1][i];
                    const beneficiary = this.currentAuctionNFTsResult[2][i];
                    const startingPrice = Number(web3.utils.fromWei(this.currentAuctionNFTsResult[3][i], 'ether'));
                    const highestBidder = this.currentAuctionNFTsResult[4][i];
                    const highestPrice = Number(web3.utils.fromWei(this.currentAuctionNFTsResult[5][i], 'ether'));
                    const auctionEndTime = this.currentAuctionNFTsResult[6][i];
                    const nftDuration = this.currentAuctionNFTsResult[7][i];
                    const refundPending = Number(web3.utils.fromWei(this.currentAuctionNFTsResult[8][i], 'ether'));
                    this.auctionedNFTsInfoMap[nftId] = {
                        ApId,
                        beneficiary,
                        startingPrice,
                        highestBidder,
                ...
                if (this.allAuctionNFTNumber === 0) {
                    this.noAuctionNFT = true;
                }
                console.log(this.auctionedNFTsInfoMap)
            } else {
                console.error("Invalid result structure:", this.ownedNFTsResult);
                Swal.fire({
                    icon: "error",
                    title: "Network Error!",
                    text: "Failed to get auction information!",
                });
            }
        },
